...
const APP_VERSION = '2025.10.8.1759926313029';
const CACHE_NAME = 'pizzalab-v2025.10.8.1759926313029';
const urlsToCache = [
  '/',
  '/orders',
  '/admin',
...
self.addEventListener('fetch', (event) => {
  ...
  // IMPORTANT: Skip Supabase API calls to prevent mobile database issues
  if (event.request.url.includes('supabase.co') || 
      event.request.url.includes('foymsziaullphulzhmxy.supabase.co')) {
    console.log('🔄 Bypassing service worker for Supabase API:', event.request.url);
    return; // Let the browser handle Supabase requests directly
  }

  // Skip other API calls that should not be cached
  if (event.request.url.includes('/api/') || 
      event.request.url.includes('api.') ||
      event.request.url.includes('.json') && event.request.url.includes('http')) {
    return;
  }
