...
CREATE POLICY "Allow authenticated users to upload files" 
  ON storage.objects 
  FOR INSERT 
  WITH CHECK (bucket_id IN ('uploads', 'admin-uploads', 'gallery', 'specialties') AND auth.role() = 'authenticated');

DROP POLICY IF EXISTS "Allow authenticated users to update files" ON storage.objects;
CREATE POLICY "Allow authenticated users to update files" 
  ON storage.objects 
  FOR UPDATE 
  USING (bucket_id IN ('uploads', 'admin-uploads', 'gallery', 'specialties') AND auth.role() = 'authenticated')
  WITH CHECK (bucket_id IN ('uploads', 'admin-uploads', 'gallery', 'specialties') AND auth.role() = 'authenticated');
