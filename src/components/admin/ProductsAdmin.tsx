...
const ProductsAdmin = () => {
  ...
  const generateUniqueSlug = async (name: string, excludeId?: string) => {
    ...
    console.log(`[generateUniqueSlug] Starting with baseSlug: "${baseSlug}", excludeId: "${excludeId}"`);

    while (true) {
      console.log(`[generateUniqueSlug] Checking slug: "${slug}"`);
      ...
      const { data, error } = await supabase
        ...
        .select('id')
        .eq('slug', slug)
        .neq('id', excludeId || ''); // Exclude current product when editing
      ...
      console.log(`[generateUniqueSlug] Found ${data?.length || 0} products with slug "${slug}"`);

      // If no products found with this slug, it's unique
      if (!data || data.length === 0) {
        console.log(`[generateUniqueSlug] Slug "${slug}" is unique!`);
        break;
      }
      ...
      counter++;
      console.log(`[generateUniqueSlug] Slug exists, trying: "${slug}"`);
    }

    console.log(`[generateUniqueSlug] Final slug: "${slug}"`);
    return slug;
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      console.log(`[handleSubmit] Starting submission for product: "${formData.name}"`);

      // Generate unique slug
      const uniqueSlug = await generateUniqueSlug(formData.name, editingProduct?.id);
      console.log(`[handleSubmit] Generated unique slug: "${uniqueSlug}"`);
      ...
      console.log(`[handleSubmit] Base product data:`, baseProductData);
